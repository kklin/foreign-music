  ...
  spotifyApi.clientCredentialsGrant().then(
    ...
    genres.forEach((genre) => {
      nationalities.forEach((nationality) => {
        ...
        const promise = spotifyApi.searchPlaylists(`${nationality} ${genre}`).then((resp) => {
          // Get 5 random playlists.
          ...
          return Promise.all(playlistSample.map((playlist) => {
            return pgClient.query('INSERT INTO playlists(id, owner_id, name, genre, country) VALUES ($1, $2, $3, $4, $5)',
              [playlist.id, playlist.owner.id, playlist.name.slice(0,128), genre, nationality]).then(() => {
    ...
    Promise.all(promises).then(() => {
      pgClient.end().then(() => console.log('closed'));
    });
